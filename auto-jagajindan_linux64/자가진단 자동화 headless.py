...
#pa = input('비번 입력:')
while True:
        if str(time.strftime('%H:%M', time.localtime(time.time()))) == '07:59':
            print('07:59')
            break
        time.sleep(60)
while True:
    #print(datetime.now())
    while True:
        if str(time.strftime('%H:%M', time.localtime(time.time()))) == '08:00':
            print('08:00')
            break
        time.sleep(1)
    time.sleep(2)
    if dayoff() == True: #False:    
        ...
        driver = webdriver.Chrome('auto-jagajindan_linux64/chromedriver')# 'chromedriver.exe', options =  webdriver_options )
        ...
        driver.quit()
        print('브라우저 종료')
        time.sleep(86360) #23시간59분20초 정지
    elif dayoff() == True: #False:
        time.sleep(86398)
