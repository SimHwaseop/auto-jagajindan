...
아직 해결하지 못한 문제가 있다. 그것은 정보가 바뀌었다는 것을 알려주는 프레임을 만드는 것인데 다른 프레임의 변수를 가져오는 것(다른 클래스)을 구글링 하여 모든 예제를 여기에 사용해보았지만 문제는 해결하지 못하였다   
이러한 이유들로 추측 했을떄 tkinter에 적용하는 코드가 따로 있거나 tkinter에는 적용이 불가는 하다는 것으로 생각 된다.  
